...
/**
  * Define a standard set of parameters that can be passed to a reduction
  *function.
  */
typedef struct {
   /** The 'fill value' of the input data (i.e. the value that indicates a
    *particular observation should be discarded) */
   NUMERIC_WORKING_TYPE input_fill_value;

   /** The 'fill value' of the output data (i.e. the value that indicates that
    *there was insufficient data for a particular grid cell */
   NUMERIC_WORKING_TYPE output_fill_value;
} reduction_attrs;

/**
  * Define a standard interface for a reduction function - the name, type, and a
  *callable function pointer.
  */
typedef struct {
   /** The name of this reduction function (use only alphanumerics and
    *underscores).*/
   char* name;
   ...
   /** The actual function call
     *
     * @param set The result set of observations which this function should
     *reduce.
     * @param attrs A reduction_attrs instance.
     * @param bounds The dimension_bounds for the cell this function is
     ...
     * @param output_index The index in the output array where the reduced value
     *should be stored.
