/**
  ...
  * Caspian is written in a largely object-oriented fashion (although
  *implemented in C, a language with no explicit object-orientation support); it
  *consists of a number of interfaces and implementations of those interfaces.
  *This documentation is intended to describe both interfaces and current
  *implementations, as well as to provide a guide for providing new
  *implementations of various interfaces.
  * @section style_sec Code Style
  * A typical aspect of Caspian (such as a grid, index or reduction function) is
  *described using a struct, with members holding both data and function
  *pointers. Many  of these structs will contain an opaque (void) pointer for
  *storage of implementation-specific information. An implementation (for
  *example, the kdtree implementation of an index) will normally provide a
  *factory function, which allocates and initialises the struct, assigning the
  *implementation's implementation of each method to the appropriate function
  *pointer, and then returns the struct. In addition, each struct will contain
  *an implementation-specific method to free or close the resources associated
  *with that struct, such that each struct may be handled in a generic fashion
  *throughout its lifetime.
  * @section style_structure Structure
  * Input data is read by a coordinate_reader, which projects the data using a
  *\ref projector, and the results are stored in an \ref spatial_index. A \ref
  *grid is then iterated over, querying the \ref spatial_index for a result_set
  *of observations that fall within that \ref grid cell. The result_set is
  *reduced to a single value by a reduction_function, and saved to the output.
  *
  * By default, coordinate_reader is implemented by \ref
  *rawfile_coordinate_reader.h "a raw file backed reader", \ref projector is
  *implemented by \ref proj_projector.h "the PROJ.4 library", and \ref
  *spatial_index is implemented by \ref kd_tree.h "an adaptive kd-tree". A
  *number of implementations of a reduction_function are included, namely \ref
  *reduce_numeric_mean "mean", \ref reduce_numeric_weighted_mean
  *"distance-weighted mean", \ref reduce_numeric_median "median", \ref
  *reduce_coded_nearest_neighbour "nearest-neighbour", and \ref
  *reduce_numeric_newest "newest".
  * @section section_implementation Implementation Details
  * Caspian is designed to handle data of various types and sizes, but to reduce
  *the burden on reduction_function implementers, a system for representing and
  *manipulating data of different types has been created. The types are split
  *into two broad classes - data with a numeric interpretation, and data that
  *should be handled opaquely (i.e. a collection of bits that must be
  *preserved); these are known as numeric and coded data respectively.
  *
  * Numeric data is internally transformed into the data type represented by
  *#NUMERIC_WORKING_TYPE - a floating point type of suitable precision for the
  *machine architecture. By retrieving numeric input data using \ref
  *numeric_get, an implementor of a reduction function can manipulate numeric
  *data of any type using the #NUMERIC_WORKING_TYPE representation. Similarly,
  *the reduction_function need only produce output in #NUMERIC_WORKING_TYPE
  *format, and this will be automatically converted to the correct output
  *representation.
  *
  * Coded data is merely given a unit size, and blocks of this data are copied
  *around with no regard to the contents of each block. Reduction functions use
  *\ref coded_get and \ref coded_put to retrieve the input data and store the
  *output data.
  *
  * To facilitate the handling of these different types of data, the \ref dtype
  *struct represents all information pertaining to a data type - this may be
  *used by reduction functions to, for example, allocate a temporary storage
  *area for coded data by using the dtype::size field to determine the amount of
  *storage necessary.
  */
...
/**
  * Polar circumference of the earth according to WGS84 - used for calculating
  *default values
  */
#define WGS84_POLAR_CIRCUMFERENCE 40007863.0
/**
  * Equatorial circumference of the earth according to WGS84 - used for
...
/**
  * Implementation of the gridding flow: Parse arguments, open files,
  *build/load/save index, run gridding
...
int main(int argc, char **argv) {
   ...
   if (sizeof(float32_t) != 4 || sizeof(float64_t) != 8) {
      printf(
         "Unsupported system: 'float' and 'double' not 4 and 8 bytes respectively\n");
   }
   ...
   reduction_function selected_reduction_function =
      get_reduction_function_by_name("mean");
   ...
   #define save_optarg_string(x) x = calloc(strlen(optarg) + 1, sizeof(char)); \
   if (x == NULL) { fprintf(stderr, \
                            "Failed to allocate space to store option string"); \
                    exit(EXIT_FAILURE); \
   }; strcpy(x, optarg)
   ...
   while((curarg =
      ...
      case 'V':
         ...
         if (vertical_resolution <= 0.0) {
            fprintf(stderr,
                    "Vertical resolution must be a positive number (got %f)\n",
                    vertical_resolution);
            exit(EXIT_FAILURE);
         }
         break;
      case 'H':
         ...
         if (horizontal_resolution <= 0.0) {
            fprintf(
               stderr,
               "Horizontal resolution must be a positive number (got %f)\n",
      ...
      case 'S':
         ...
         if (vertical_sampling <= 0.0) {
            fprintf(
               stderr,
               "Vertical sampling resolution must be a positive number (got %f)\n",
               vertical_sampling);
            exit(EXIT_FAILURE);
         }
         break;
      case 's':
         ...
         if (horizontal_sampling <= 0.0) {
            fprintf(
               stderr,
               "Horizontal sampling resolution must be a positive number (got %f)\n",
               horizontal_sampling);
            exit(EXIT_FAILURE);
         }
         break;
   ...
   // Check we have required options - required options depends on mode of
   // operation
   if (!loading_index) {
   ...
   if (loading_index) {
      ...
      projector *input_projection = get_proj_projector_from_string(
         projection_string);
      ...
      coordinate_reader *reader = get_coordinate_reader_from_files(
         input_lat_filename, input_lon_filename, input_time_filename,
         input_projection);
      if (reader == NULL) {
      ...
      if (verbosity >
          0) printf("Building index took %d seconds\n",
   ...
   if (generating_image) {
      ...
      unsigned int input_data_number_bytes = data_index->num_observations *
                                             input_dtype.size;
      unsigned int output_data_number_bytes = width * height *
                                              output_dtype.size;
      unsigned int output_geo_number_bytes = width * height * sizeof(float32_t);

      memory_mapped_file *data_input_file = NULL, *data_output_file = NULL,
      *latitude_output_file = NULL, *longitude_output_file = NULL;
      ...
      out.grid_spec =
         initialise_grid(width, height, vertical_resolution,
                         horizontal_resolution,
                         vertical_sampling, horizontal_sampling, central_x,
                         central_y,
                         data_index->input_projector);
      ...
      if (write_data) {
         data_input_file = open_memory_mapped_input_file(
            input_data_filename, input_data_number_bytes);
         data_output_file = open_memory_mapped_output_file(
            output_data_filename, output_data_number_bytes);
      ...
      if (write_lats) {
         latitude_output_file = open_memory_mapped_output_file(
            output_lat_filename, output_geo_number_bytes);
         out.lats_output =
            (float32_t *) latitude_output_file->memory_mapped_data;
      } else {
         out.lats_output = NULL;
      }

      if (write_lons) {
         longitude_output_file = open_memory_mapped_output_file(
            output_lon_filename, output_geo_number_bytes);
         out.lons_output =
            (float32_t *) longitude_output_file->memory_mapped_data;
      } else {
      ...
      perform_gridding(in, out, selected_reduction_function, &r_attrs,
                       verbosity);
      time_t gridding_end_time = time(NULL);
      if (verbosity >
          0) printf("Gridding took %d seconds\n",
