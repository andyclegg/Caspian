/**
  * @file
  *
  * Defines data structures for representing data types and establishes the
  *numeric working type
  */
...
/**
  ...
  */
typedef enum {uint8, uint16, uint32, uint64, int8, int16, int32, int64, float32,
              float64, coded8, coded16, coded32, coded64, undef_type} dtype_t;
...
#ifdef INT64_MAX
/** Define a numeric working type as float32_t or float64_t on 32-bit and 64-bit
...
/**
  ...
  *
  * Dimension bounds are given as an array of floating point numbers. For each
  *dimension, a lower and upper bound are required. The numbers are ordered as
  *{x-lower, x-upper, y-lower, y-upper, t-lower, t-upper} (and this may be
  *extended for more dimensions if necessary).
  */
...
void numeric_put(void *data, dtype output_dtype, int index,
                 NUMERIC_WORKING_TYPE data_item);
