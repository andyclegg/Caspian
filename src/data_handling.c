/**
  * @file
  *
  * Implements methods for retrieving and storing data of different types from
  *untyped memory arrays.
  */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/**
  * Get a single number from an array, formatted as the current working numeric
  *type
  ...
  * @return The number from the array, formatted according to
  *NUMERIC_WORKING_TYPE (normally a float or double)
  */
...
void numeric_put(void *data, dtype output_dtype, int index,
   ...
   default:
      fprintf(stderr,
              "Unknown dtype '%s' (this is probably a bug in Caspian).\n",
              output_dtype.string);
      exit(EXIT_FAILURE);
   }
}
...
/**
  ...
  *
  * The dtype string is constructed as type + size, where size is 8, 16, 32 or
  *64, and type may be one of:
