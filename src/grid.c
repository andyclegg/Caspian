...
/**
  ...
  *
  * The sampling offsets are initialised to the given value divided by 2 if
  *provided (e.g. vsample or hsample are not 0). Otherwise,
  ...
  * @param input_projector An initialised projector which transforms spherical
...
grid *initialise_grid(int width, int height, float vertical_resolution,
   ...
   // Set sampling factor offset to be equal to resolution/2 if not set,
   // otherwise to provided value/2
   if (hsample == 0.0) {
